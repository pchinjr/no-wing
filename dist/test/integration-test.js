...
export class IntegrationTestRunner {
    ...
    async runEndToEndTests() {
        ...
        suite.tests.push(await this.runTest('Complete Deployment Workflow', async () => {
            ...
            const report = await this.auditLogger.generateComplianceReport(new Date(Date.now() - 60 * 60 * 1000), new Date());
    ...
    generateTestReport() {
        ...
        // Show failed tests
        const failedTests = this.testResults.flatMap(suite => suite.tests.filter(test => !test.success).map(test => ({ suite: suite.name, ...test })));
