...
async function testPermissionManagement() {
    ...
    try {
        ...
        try {
            ...
            await auditLogger.logRoleAssumption('arn:aws:iam::123456789012:role/no-wing-test-role', 'test-session', true);
            console.log('✅ Role assumption logged');
            // Test AWS operation logging
            await auditLogger.logAWSOperation('s3', 'GetObject', ['arn:aws:s3:::test-bucket/test-key'], { Bucket: 'test-bucket', Key: 'test-key' }, true);
            console.log('✅ AWS operation logged');
            // Test permission request logging
            await auditLogger.logPermissionRequest('s3-operations', ['s3:GetObject', 's3:PutObject'], ['arn:aws:s3:::test-bucket/*'], 'Testing permission request logging', 'test-req-123', 'pending');
        ...
        try {
            ...
            await auditLogger.logAWSOperation('cloudformation', 'CreateStack', deployContext.resources || [], { StackName: 'no-wing-integration-test' }, result.success, result.success ? undefined : result.message);
            // 4. Generate final report
            const finalReport = await auditLogger.generateComplianceReport(new Date(Date.now() - 60 * 60 * 1000), // Last hour
            new Date());
