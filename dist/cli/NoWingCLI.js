...
export class NoWingCLI {
    ...
    async initialize() {
        ...
        this.permissionElevator = new PermissionElevator(this.credentialManager, this.roleManager, this.configManager);
        this.auditLogger = new AuditLogger(this.credentialManager);
        this.deploymentManager = new DeploymentManager(this.credentialManager, this.clientFactory, this.permissionElevator, this.auditLogger, this.roleManager);
        try {
    ...
    async handleDeploy(template, options) {
    ...
    async handleRollback(stackName, options) {
    ...
    async handleCredentialSwitch(context) {
    ...
    async handleListRoles(options) {
        console.log('🎭 Available Roles:');
        try {
            const roles = await this.roleManager.listAvailableRoles();
            let filteredRoles = roles;
            if (options.pattern) {
                filteredRoles = roles.filter(role => role.roleName.toLowerCase().includes(options.pattern.toLowerCase()));
            }
    ...
    async handlePermissionRequests(options) {
    ...
    async handleAuditEvents(options) {
    ...
    async handleAuditReport(options) {
...
//# sourceMappingURL=NoWingCLI.js.map