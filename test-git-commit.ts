...
async function main() {
  console.log("Starting test-git-commit.ts");
  
  const gitManager = new GitIdentityManager({
    agentName: "test-agent",
    authorName: "Test Agent",
    authorEmail: "test-agent@no-wing.local",
  });
  
  try {
    // Make the commit
    console.log("Making commit...");
    const commitHash = await gitManager.commit("Test commit from test-git-commit.ts");
